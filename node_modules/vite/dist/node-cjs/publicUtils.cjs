...
const { version: version$1 } = JSON.parse(
  fs.readFileSync(new URL("../../package.json", (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('node-cjs/publicUtils.cjs', document.baseURI).href)))).toString()
);
...
const _dirname = path.dirname(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('node-cjs/publicUtils.cjs', document.baseURI).href))));
...
function windowsMappedRealpathSync(path2) {
  const realPath = fs.realpathSync.native(path2);
...
var getEtag = /*@__PURE__*/getDefaultExportFromCjs(etagExports);
...
function loadEnv(mode, envDir, prefixes = "VITE_") {
  ...
  const parsed = Object.fromEntries(
    envFiles.flatMap((filePath) => {
      ...
      return Object.entries(mainExports$1.parse(fs.readFileSync(filePath)));
    })
  );
  ...
  mainExports.expand({ parsed, processEnv });
